...
<?php include 'header.php';?>
  <div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <h2><i class="fa fa-cubes fa-fw"></i>Sale Report by Detail</h2>
        </div>
    </div>
    <br>
    <div class="row">
                ...
                <tbody>
                    <?php 
                        if(isset($_POST['btn_search'])){
                            ...
                            if($v_sale_type != ''){
                                $data_query = $connect->query("SELECT A.* FROM tbl_order_detail AS A WHERE DATE_FORMAT(od_submit_date_time,'%Y-%m-%d') BETWEEN '$v_from' AND '$v_to' AND A.od_client_id='$v_sale_type' ORDER BY od_submit_date_time DESC");
                            }else{
                                $data_query = $connect->query("SELECT A.* FROM tbl_order_detail AS A WHERE DATE_FORMAT(od_submit_date_time,'%Y-%m-%d') BETWEEN '$v_from' AND '$v_to' ORDER BY od_submit_date_time DESC");
                            }
                            
                        }else{
                            $current_date = date('Y-m-d');
                            $data_query = $connect->query("SELECT A.* FROM tbl_order_detail AS A WHERE DATE_FORMAT(od_submit_date_time,'%Y-%m-%d')='$current_date' ORDER BY od_submit_date_time DESC");
                        }
                        $no = 0;
                        $sum_qty = 0;
                        $sum_price = 0;
