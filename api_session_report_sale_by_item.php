...
<div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <h2><i class="fa fa-cubes fa-fw"></i>Session Sale Report by Items</h2>
        </div>
    </div>
    <br>
    <div class="row">
                ...
                <tbody>
                    <?php 
                        if(isset($_POST['btn_search'])){
                            ...
                            $data_query = $connect->query("SELECT *,SUM(od_amount) AS s_qty,SUM(od_amount*od_price*od_amount*od_discount/100) AS s_discount FROM tbl_order_detail WHERE od_submit_date_time BETWEEN '$v_from' AND '$v_to' GROUP BY od_product_id ORDER BY od_submit_date_time DESC");
                        }else{
                            $current_date = date('Y-m-d');
                            $data_query = $connect->query("SELECT *,SUM(od_amount) AS s_qty,SUM(od_amount*od_price*od_amount*od_discount/100) AS s_discount FROM tbl_order_detail WHERE DATE_FORMAT(od_submit_date_time,'%Y-%m-%d') = '$current_date' GROUP BY od_product_id ORDER BY od_submit_date_time DESC");
                        }
                        $no = 0;
                        $sum_qty = 0;
                        $sum_discount = 0;
