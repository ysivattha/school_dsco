...
if(isset($_POST['search'])){
  ...
  if(@$_POST['txt_product'] != ""){
    ...
    $sql = "SELECT * FROM tbl_wh_stock_out AS A
                    LEFT JOIN product AS B ON A.whso_product_code=B.pro_id
                    LEFT JOIN employee AS C ON A.whso_employee=C.emp_id
                    LEFT JOIN tbl_unit AS D ON D.u_id=A.whso_unit
                    WHERE A.whso_date_record BETWEEN '$from' AND '$to' AND A.whso_product_code='$v_product'
                    ORDER BY A.whso_date_record DESC      ";  
    ...
    $sql = "SELECT * FROM tbl_wh_stock_out AS A
                    ...
                    LEFT JOIN employee AS C ON A.whso_employee=C.emp_id
                    LEFT JOIN tbl_unit AS D ON D.u_id=A.whso_unit
                    WHERE A.whso_date_record BETWEEN '$from' AND '$to'
                    ORDER BY A.whso_date_record DESC      ";  
    $result = $connect->query($sql);
  }
}else{
  ...
  $sql = "SELECT * FROM tbl_wh_stock_out AS A
                    ...
                    LEFT JOIN employee AS C ON A.whso_employee=C.emp_id
                    LEFT JOIN tbl_unit AS D ON D.u_id=A.whso_unit
                    WHERE DATE_FORMAT(A.whso_date_record,'%Y-%m')='$v_date'
                    ORDER BY A.whso_date_record DESC      ";  
  $result = $connect->query($sql);
}
  ...
  if(isset($_POST["btnadd"])){
    ...
    $v_note = $_POST["txt_note"];
     ...
     $sql = "INSERT INTO tbl_wh_stock_out 
                          (whso_date_record
                            ,whso_letter_no
                            ,whso_product_code
                            ,whso_qty_out
     ...
     header('location: wh_stock_out.php?message=success');
 }
    if(isset($_GET["id"])){
    $id = $_GET["id"];
                    ...
                    <div class="col-md-12">
                        <form method="post" enctype="multipart/form-data" action="">     
                          ...
                          <div class="form-group col-xs-12">
                            ...
                            <input class="form-control" required  name="txt_qty_out" type="text" placeholder="qty in..">          
                          </div>
                          <div class="form-group col-xs-6">
                            <label for ="">Received From:</label>                                          
                            <input class="form-control" required  name="txt_sent_to" type="text" placeholder="received from..">          
                          </div>
                          <div class="form-group col-xs-6">
                                    ...
                                    <tbody>
                                        <?php
                                          ...
                                          <tr>
                                            ...
                                            <td align = "center">
                                            <a href="edit_wh_stock_out.php?id=<?php echo $row['whso_id']; ?>" class="btn btn-primary"><i class="fa fa-edit"></i></a>
                                            <a onclick = "return confirm('Are you sure to delete ?');" href="wh_stock_out.php?id=<?php echo $row['whso_id']; ?>" class="btn btn-danger"><i class="fa fa-trash"></i></a>
                                            </td>
                                          </tr> 
                                          <?php
                                            }  
